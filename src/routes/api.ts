...
import { fetchQuotes, fetchAllQuotes, searchQuotes } from "../quotes";
...
router.get("/quotes/:amount([0-9]+)", async (req, res) => {
  ...
  } else if (requested_amount <= FETCH_LIMIT && requested_amount > 0) {
    const quotes = fetchQuotes(requested_amount);
    const return_obj = {status: 200, data: quotes};
    return res.status(200).json(return_obj);
  } else {
    return res.status(400).json({status: 400, error: "Fetch amount has to be larger than 0 and less than or equal to 10!"});
  }
});
